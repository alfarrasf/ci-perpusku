    ...
    <div class="container">
      <form>
        <div class="form-group row">
          ...
          <div class="col-sm-10">
          <input type="text" class="form-control form-control-sm" id="kode" placeholder="Enter Kode">
          </div>
        </div>
        <div class="form-group row">
    ...
    <script type="text/javascript">
      $(document).ready(function() {
        ...
        function show_subs() {
          $.ajax({
            type : "AJAX",
            url : "<?php echo site_url('master/supplier_data'); ?>",
            dataType : "JSON",
            success : function(data) {
              ...
              for(i=0; i<data.length; i++) {
                ...
                html += '<tr>' +
                        ...
                        '<td>' + data[i].telp + '</td>' +
                        '<td>' + data[i].flag + '</td>' +
                        '<td>' + '<a class="edit" data-id="' + data[i].kode + '" data-nama="' + data[i].nama + '" data-alamat="' + data[i].alamat+ '" data-telepon="'+ data[i].telp +'" style="cursor:pointer"><i class="material-icons">settings</i></a><a class="delete" data-id="' + data[i].kode + '" style="cursor:pointer"><i class="material-icons">clear</i></a><a class="flag" data-id="' + data[i].kode + '" data-flag="' + data[i].flag + '" style="cursor:pointer"><i class="material-icons">check</i></a>'+ '</td>' +  
                        '</tr>'
              }
              $('#show_data').html(html);
            }
        ...
        $('#btn-save').on('click', function() {
          ...
          $.ajax({
            type : "POST",
            url : "<?php echo site_url('master/save_supplier'); ?>",
            dataType : "JSON",
            data : {kode : kode, nama : nama, alamat : alamat, telepon : telepon, keterangan : keterangan},
            success : function() {
              $('[name="kota"]').val("");
              show_subs();
            }
        ...
        $('#btn-edit-save').on('click', function() {
          ...
          $.ajax({
            type : "POST",
            url : "<?php echo site_url('master/edit_supplier'); ?>",
            dataType : "JSON",
        ...
        $('#btn-delete').on('click', function() {
          ...
          $.ajax({
            type : "POST",
            url : "<?php echo site_url('master/delete_supplier'); ?>",
            dataType : "JSON",
            data : {id : id},
            success : function() {
        ...
        $('#show_data').on('click','.flag',function(){
          ...
          $.ajax({
            type : "POST",
            url : "<?php echo site_url('master/flag_supplier'); ?>",
            dataType : "JSON",
